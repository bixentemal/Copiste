...
/// `Bundle.module`'s generated accessor only ever finds this resource bundle at the build
/// machine's own absolute `.build` path or at an app bundle's top level — never at
/// `Contents/Resources`, where a packaged, signed `.app` actually puts it — so touching it
/// crashes every time outside the original dev machine. Look in the right place ourselves.
enum KeyboardShortcutsResources {
